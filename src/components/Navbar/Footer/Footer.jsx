import styles from './Footer.module.css'

const Footer = () => {
    return (
        <div className={styles.container}>
            <div className={styles.redes}>
                <div className={styles.red}>
                    <img src="data:image/png;base64,"/>
                    <p><a target='blank' href="https://www.linkedin.com/in/federico-herrera-828636244/"></a>LinkedIn</p>
                </div>
                <div className={styles.red}>
                    <img src="data:image/png;base64,"/>
                    <p>fede.juan.herrera@gmail.com</p>
                </div>
                <div className={styles.red}>
                    <img src="data:image/png;base64,"/>
                    <p><a target='blank' href="https://www.instagram.com/fedeherreraa_/">Instagram</a></p>
                </div>
            </div>

        </div>
    )

}

export default Footer;